...
// 1. ĐÃ THÊM HÀM formatCredits ĐỂ SỬA LỖI TRẮNG TRANG
...
export default function BillingPage() {
    ...
    axios.get(`${API_BASE_URL}/api/auth/stats`, { headers })
      .then((r) => {
        if (r.data.success) {
          ...
          // 2. ĐÃ SỬA setTokensUsed THÀNH setCreditsUsed
          setCreditsUsed(r.data.data.tokens_used); 
        }
      })
      .catch(() => {});
  ...
  // 3. ĐÃ SỬA tokensUsed THÀNH creditsUsed
  const usedPct = credits != null && (creditsUsed + credits) > 0
    ? Math.round((creditsUsed / (creditsUsed + credits)) * 100)
    : 0;
