...
-- 3. Atomic weighted-token increment — tránh race condition khi nhiều jobs chạy song song
CREATE OR REPLACE FUNCTION increment_user_tokens(p_user_id uuid, p_tokens integer, p_credits numeric)
RETURNS void
LANGUAGE sql
AS $$
  UPDATE users
  SET tokens_used = tokens_used + p_tokens,
      credits     = GREATEST(0, credits - p_credits)
  WHERE id = p_user_id;
$$;
