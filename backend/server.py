...
router.post('/run-test', async (req, res) => {
  ...
  // Chạy pipeline bất đồng bộ (không block API response)
  setImmediate(async () => {
    try {
      // Thay thế bằng module pipeline thực tế của bạn
      // const { AIPipelineOrchestrator } = require('./pipeline_AI');
      // const pipeline = new AIPipelineOrchestrator({ userId: user_id, projectId: project_id, sourceCodePath: source_path });
      // await pipeline.executePipeline();
      console.log(`Pipeline đang chạy cho user: ${user_id}, project: ${project_id}`);
    } catch (err) {
      console.error('Lỗi pipeline:', err.message);
    }
  });

  return res.json({
    status: 'success',
    message: `Pipeline đã được kích hoạt cho user ${user_id}, project ${project_id}`,
